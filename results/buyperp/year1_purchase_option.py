...
# How much trailing history feeds the sigma/mu estimates below (see the next
# markdown cell) — a shorter window reacts faster to a recent regime change,
# a longer one is less noisy. 3 is the ceiling, not a magic number: it's just
# how far back results/buyperp/metals_prices.csv currently reaches
# (metals.lookback_years in config/settings.yaml).
...
# historical returns, using the trailing `VOLATILITY_LOOKBACK_YEARS` window
# (not necessarily the full history — see the Configuration cell). Using mu
# instead of a synthetic risk-neutral rate is deliberate: see the next
# markdown cell. The EUR/USD rate gets the exact same treatment (its own
# sigma/mu from its own history), so it can later be simulated forward just
# like the metal cost. `S0`/`fx0` themselves always use the single latest
# observation, regardless of the lookback window used for sigma/mu.
...
sigma = bp.annualized_volatility(recent_metals["basket_cost_usd"])
mu = bp.annualized_drift(recent_metals["basket_cost_usd"])
...
# uses `mu` (estimated straight from our collected data) to drive
...
print(f"Average exercise month (1=Jan .. 12=Dec): {adaptive_mc['mean_exercise_month']:.1f}")
